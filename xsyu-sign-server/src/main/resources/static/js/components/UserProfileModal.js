...
Vue.component('user-profile-modal', {
    ...
    watch: {
        visible(newVal) {
            if (newVal) {
                ...
                this.signDaysConfig = this.userInfo.signDays
                    ? this.userInfo.signDays.split(',').map(Number).filter(n => !isNaN(n))
                    : [0, 1, 2, 3, 4, 5, 6];
            }
        }
    },
    methods: {
        toggleSignDay(day) {
            ...
            if (index > -1) this.signDaysConfig.splice(index, 1);
            else this.signDaysConfig.push(day);
    ...
    template: /*html*/ `
        <base-modal :visible="visible" title="修改个人信息" @close="$emit('close')">
